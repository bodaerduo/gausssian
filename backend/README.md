...
1. 视频先由 FFmpeg 按质量档位限制在 1280×720 / 1280×720 / 1920×1080 像素（快速/推荐/高质量，保持比例），再按 3/4/6 FPS 抽帧；Brush 训练分辨率默认为 720/720/1080，训练步数默认为 12k/30k/50k，最大 Splat 默认为 2.5M/5M/8M；建模接口支持通过表单参数覆盖这些默认值，可通过 `GAUSSIAN_VIDEO_MAX_DIMENSION` 设置更低的全局上限以进一步提速；
