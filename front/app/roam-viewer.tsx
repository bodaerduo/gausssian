...
export function RoamViewer({ modelUrl, modelName, onClose }: { modelUrl: string; modelName: string; onClose: () => void }) {
  ...
  return <div className="roam-overlay" role="dialog" aria-modal="true" aria-label={`${modelName} 漫游模式`}><div className="roam-shell"><div className="roam-canvas"><div className="roam-engine" ref={mountRef} /><div className="roam-crosshair" /><div ref={eagleRef} className={`eagle-avatar eagle-speed-${speedLevel}`} data-moving="false" data-turn="none" aria-hidden="true"><Image src="/eagle-flight.png" alt="" width={170} height={142} unoptimized /></div><div className="roam-status"><span className={loaded ? 'status-pip' : 'status-pip amber'} />{error ? error : loaded ? '真实 PLY 场景已载入' : '正在加载真实 Gaussian 场景…'}</div></div><div className="roam-cockpit"><div className="roam-controls"><div className="virtual-keyboard"><div className="key-row">{virtualKey('w')} {virtualKey('a')} {virtualKey('s')} {virtualKey('d')}</div><div className="key-row">{virtualKey('q')} {virtualKey('e')} {virtualKey('r')} {virtualKey('f')}</div><div className="key-row key-row-wide">{virtualKey('shift', 'SHIFT')}</div></div><div className="speed-control" aria-live="polite"><span>速度</span><strong>{speedLabels[speedLevel]}</strong><small>点击 SHIFT 切换</small></div></div></div></div></div>;
}
