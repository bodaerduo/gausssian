...
sed -i \
  ...
  -e "s|^CUBECL_WGPU_DEFAULT_DEVICE=.*|CUBECL_WGPU_DEFAULT_DEVICE=${CUBECL_WGPU_DEFAULT_DEVICE:-DiscreteGpu(0)}|" \
  "$BACKEND_DIR/.env"
if ! grep -q '^CUBECL_WGPU_DEFAULT_DEVICE=' "$BACKEND_DIR/.env"; then
  printf '\nCUBECL_WGPU_DEFAULT_DEVICE=%s\n' "${CUBECL_WGPU_DEFAULT_DEVICE:-DiscreteGpu(0)}" >> "$BACKEND_DIR/.env"
fi
